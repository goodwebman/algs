...
feed;   // → ['d', 'c', 'b']`,
        ...
        },
        {
          ...
          examples: [
            `// ГЛАВНОЕ ОТЛИЧИЕ ОТ slice: второй аргумент — не «докуда», а «СКОЛЬКО ШТУК».
// И splice меняет исходный массив, а возвращает то, что вырезал.
//
//   0   1   2   3   4   5
//   │ a │ b │ c │ d │ e │
//       ▲ start = 1
//       └───────┘  deleteCount = 2  → вырезаем ['b', 'c']
//
// после операции сам массив:
//   │ a │ d │ e │

const arr = ['a', 'b', 'c', 'd', 'e'];

arr.splice(1, 2);   // → ['b', 'c'] — ВОЗВРАТ: удалённое, а не остаток
arr;                // → ['a', 'd', 'e'] — исходник изменён на месте`,
            `// те же цифры (1, 3) в slice и splice дают разные куски
const src = ['a', 'b', 'c', 'd', 'e'];

//       └───────────┘  splice(1, 3) съедает ['b', 'c', 'd']
//       └───────┘  а slice(1, 3) берёт только ['b', 'c']

src.slice(1, 3);   // → ['b', 'c'] — до позиции 3
src;               // → ['a', 'b', 'c', 'd', 'e'] — цел

const copy = ['a', 'b', 'c', 'd', 'e'];
copy.splice(1, 3);   // → ['b', 'c', 'd'] — три штуки от позиции 1
copy;                // → ['a', 'e']`,
            `// вставка без удаления: deleteCount = 0, новые элементы встают ПЕРЕД start
//
//   0   1   2   3
//   │ a │ b │ c │
//       ▲ start = 1 — вставляем сюда, перед 'b'
//
// после:
//   │ a │ X │ Y │ b │ c │
...
arr.splice(1, 0, 'X', 'Y');   // → [] — ничего не удалили
arr;                          // → ['a', 'X', 'Y', 'b', 'c']

// в конец — start равен длине; в начало — start 0
const tail = ['a', 'b'];
tail.splice(tail.length, 0, 'z');
tail;   // → ['a', 'b', 'z']

const head = ['a', 'b'];
head.splice(0, 0, 'z');
head;   // → ['z', 'a', 'b']`,
            `// замена: удалить n и вставить сколько угодно — длины могут не совпадать
//
//   │ a │ b │ c │      splice(1, 1, 'X')       → │ a │ X │ c │
//   │ a │ b │ c │      splice(1, 1, 'X', 'Y')  → │ a │ X │ Y │ c │
...
two.splice(1, 1, 'X', 'Y');
two;          // → ['a', 'X', 'Y', 'c']
two.length;   // → 4 — массив стал длиннее`,
            `// ГЛАВНАЯ ЛОВУШКА: пропущенный deleteCount ≠ нулю
const rest = ['a', 'b', 'c', 'd'];
rest.splice(2);   // → ['c', 'd'] — БЕЗ второго аргумента режет до конца
rest;             // → ['a', 'b']

const zero = ['a', 'b', 'c', 'd'];
zero.splice(2, 0);   // → [] — а с явным нулём не делает ничего
zero;                // → ['a', 'b', 'c', 'd']

// undefined в переменной ведёт себя как ноль, а не как «до конца» —
// поэтому вычисленный deleteCount лучше подставлять явно
const maybe = ['a', 'b', 'c', 'd'];
maybe.splice(2, undefined);   // → []
maybe;                        // → ['a', 'b', 'c', 'd']`,
...
// отрицательный старт — с конца (а вот deleteCount отрицательным быть не может)
//
//   0   1   2   3   4
//   │ 1 │ 2 │ 3 │ 4 │
//  -4  -3  -2  -1
...
const tail = [1, 2, 3, 4];
tail.splice(-2);   // → [3, 4] — от предпоследнего до конца
tail;              // → [1, 2]

const one = [1, 2, 3, 4];
one.splice(-2, 1);   // → [3] — одна штука от позиции -2
one;                 // → [1, 2, 4]

const neg = [1, 2, 3, 4];
neg.splice(1, -1);   // → [] — отрицательный deleteCount = ноль, не «с конца»
neg;                 // → [1, 2, 3, 4]`,
...
move(['a', 'b', 'c'], 2, 0);   // → ['c', 'a', 'b']`,
          ...
          gotcha:
            'Второй аргумент — количество, а не граница: splice(1, 3) съедает три элемента, slice(1, 3) — два. splice(1) без второго аргумента удаляет всё до конца, а splice(1, 0) — ничего. И splice возвращает УДАЛЁННОЕ, а не остаток: строчка вида «const next = arr.splice(0, 2)» почти всегда баг.',
        },
        {
      ...
      entries: [
        {
          ...
          complexity: 'O(k)',
          examples: [
            `// индексы в slice — это позиции РАЗРЕЗОВ между элементами, а не сами элементы.
// start входит, end — нет.
//
//   0   1   2   3   4   5   ← позиции разрезов
//   │ a │ b │ c │ d │ e │
//
//       └───────┘  slice(1, 3)  → ['b', 'c']
//               └───────┘  slice(3)  → ['d', 'e']
...
// длина результата = end - start, считать элементы не нужно
['a', 'b', 'c', 'd', 'e'].slice(1, 3).length;   // → 2

// и части стыкуются без нахлёста: slice(0, i) + slice(i) = исходный массив
const src = ['a', 'b', 'c', 'd', 'e'];
[...src.slice(0, 2), ...src.slice(2)];   // → ['a', 'b', 'c', 'd', 'e']`,
            `// отрицательные считаются с конца, формула перевода: i + length
//
//   0   1   2   3   4   5
//   │ a │ b │ c │ d │ e │
//  -5  -4  -3  -2  -1      ← те же позиции с конца
//
//           └───────┘  slice(-3, -1)  = slice(2, 4)

-3 + 5;   // → 2 — длина массива 5
-1 + 5;   // → 4

['a', 'b', 'c', 'd', 'e'].slice(-3, -1);   // → ['c', 'd']
['a', 'b', 'c', 'd', 'e'].slice(-2);       // → ['d', 'e']
['a', 'b', 'c', 'd', 'e'].slice(0, -1);    // → ['a', 'b', 'c', 'd'] — всё, кроме последнего`,
            `const arr = [1, 2, 3, 4, 5];
...
src[0].n;   // → 1`,
        ...
        {
          sig: 'slice против splice — как перестать путать',
          key: null,
          summary: 'Одна буква разницы в имени, три отличия в поведении: что значит второй аргумент, мутирует ли, что возвращает.',
          examples: [
            `// один и тот же массив, одни и те же цифры (1, 3):
//
//   0   1   2   3   4   5
//   │ a │ b │ c │ d │ e │
//
//       └───────┘  slice(1, 3):  ОТ позиции 1 ДО позиции 3 → 2 элемента
//       └───────────┘  splice(1, 3): ОТ позиции 1, 3 ШТУКИ → 3 элемента

const a = ['a', 'b', 'c', 'd', 'e'];
a.slice(1, 3);   // → ['b', 'c']
a;               // → ['a', 'b', 'c', 'd', 'e'] — не тронут

const b = ['a', 'b', 'c', 'd', 'e'];
b.splice(1, 3);   // → ['b', 'c', 'd']
b;                // → ['a', 'e'] — изменён на месте`,
            `//               slice(start, end)     splice(start, count, ...items)
//  2-й аргумент  граница «до», не вкл. КОЛИЧЕСТВО элементов
//  мутирует      нет                   ДА
//  возвращает    вырезанную копию      УДАЛЁННЫЕ элементы
//  вставляет     нет                   да, с 3-го аргумента
//  без 2-го арг. до конца                до конца
//  иммутабельный сам такой              toSpliced (ES2023)

// мнемоника: в splice есть «p» — он «правит» массив на месте.
// slice только «слизывает» копию, оригинал целый.

const src = ['a', 'b', 'c'];
src.toSpliced(1, 1);   // → ['a', 'c'] — то же, что splice, но копией
src;                   // → ['a', 'b', 'c']`,
            `// три способа удалить элемент по индексу — и что с исходником
const i = 1;

const withFilter = ['a', 'b', 'c'].filter((_, idx) => idx !== i);
withFilter;   // → ['a', 'c']

const withSlice = [...['a', 'b', 'c'].slice(0, i), ...['a', 'b', 'c'].slice(i + 1)];
withSlice;    // → ['a', 'c']

const mut = ['a', 'b', 'c'];
mut.splice(i, 1);   // → ['b'] — вернулось удалённое
mut;                // → ['a', 'c'] — результат остался в самом массиве

// в React-стейте первые два, splice — только по свежей копии`,
          ],
          gotcha:
            'Самая частая ошибка — присвоить результат: const next = arr.splice(0, 3) даёт вырезанный кусок, а не обрезанный массив. Нужен «первые три» без мутации — это arr.slice(0, 3).',
        },
        {
